...
const char* kFontPresetLabels[] = {
    ...
    "Zilla Slab",
    "Aref Ruqaa",
    "Zilla Slab (OFL fallback)",
    "Aref Ruqaa (OFL fallback)"
};
...
HHOOK g_toggleKeyboardHook{};
HHOOK g_toggleMouseHook{};
HWND g_toggleWindow{};
DWORD g_toggleWindowThread{};
std::atomic<std::uint32_t> g_capturedToggleKey{};

bool IsMouseBinding(std::uint32_t virtualKey) noexcept
{
    return virtualKey == VK_MBUTTON
        || virtualKey == VK_XBUTTON1
        || virtualKey == VK_XBUTTON2;
}

bool ExactToggleModifiersMatch(const HotkeyBinding& binding) noexcept
{
...
bool HandleToggleTransition(std::uint32_t virtualKey, bool keyDown, bool keyUp) noexcept
{
    if (keyUp) {
        std::uint32_t captured = virtualKey;
        return g_capturedToggleKey.compare_exchange_strong(
            captured, 0, std::memory_order_acq_rel);
    }
    if (!keyDown
            || !g_config.toggleHotkeyEnabled
            || g_config.toggleHotkey.virtualKey != virtualKey
            || !g_toggleWindow
            || GetForegroundWindow() != g_toggleWindow
            || !ExactToggleModifiersMatch(g_config.toggleHotkey)) {
        return false;
    }
    SetEnabled(!IsEnabled());
    g_capturedToggleKey.store(virtualKey, std::memory_order_release);
    return true;
}

LRESULT CALLBACK ToggleKeyboardHook(int code, WPARAM key, LPARAM state) noexcept
{
    if (code == HC_ACTION && !IsMouseBinding(g_config.toggleHotkey.virtualKey)) {
        const bool keyUp = (static_cast<std::uintptr_t>(state) & (UINT64_C(1) << 31)) != 0;
        const bool wasDown = (static_cast<std::uintptr_t>(state) & (UINT64_C(1) << 30)) != 0;
        if (HandleToggleTransition(
                static_cast<std::uint32_t>(key), !keyUp && !wasDown, keyUp)) {
            return 1;
        }
    }
    return CallNextHookEx(g_toggleKeyboardHook, code, key, state);
}

LRESULT CALLBACK ToggleMouseHook(int code, WPARAM message, LPARAM state) noexcept
{
    if (code == HC_ACTION && IsMouseBinding(g_config.toggleHotkey.virtualKey)) {
        ...
        bool keyUp{};
        switch (message) {
        case WM_MBUTTONDOWN: virtualKey = VK_MBUTTON; keyDown = true; break;
        case WM_MBUTTONUP: virtualKey = VK_MBUTTON; keyUp = true; break;
        case WM_XBUTTONDOWN:
        case WM_XBUTTONUP: {
            const auto* input = reinterpret_cast<const MOUSEHOOKSTRUCTEX*>(state);
            if (input) {
                virtualKey = HIWORD(input->mouseData) == XBUTTON1
                    ? VK_XBUTTON1 : VK_XBUTTON2;
                keyDown = message == WM_XBUTTONDOWN;
                keyUp = message == WM_XBUTTONUP;
            }
            break;
        }
        default: break;
        ...
        if (virtualKey != 0
                && HandleToggleTransition(virtualKey, keyDown, keyUp)) {
            return 1;
        }
    }
    return CallNextHookEx(g_toggleMouseHook, code, message, state);
}

void RemoveToggleHooks() noexcept
{
    if (g_toggleKeyboardHook) UnhookWindowsHookEx(g_toggleKeyboardHook);
    if (g_toggleMouseHook) UnhookWindowsHookEx(g_toggleMouseHook);
    g_toggleKeyboardHook = nullptr;
    g_toggleMouseHook = nullptr;
    g_toggleWindow = nullptr;
    g_toggleWindowThread = 0;
    g_capturedToggleKey.store(0, std::memory_order_release);
}

void EnsureToggleHooks(HWND window) noexcept
{
    if (!window) return;
    const DWORD thread = GetWindowThreadProcessId(window, nullptr);
    if (thread == 0) return;
    if (g_toggleKeyboardHook && g_toggleMouseHook
            && g_toggleWindow == window && g_toggleWindowThread == thread) {
        return;
    }
    RemoveToggleHooks();
    HMODULE module{};
    if (!GetModuleHandleExW(
            ...
            reinterpret_cast<LPCWSTR>(&ToggleKeyboardHook),
            &module)) {
        return;
    }
    const HHOOK keyboard = SetWindowsHookExW(
        WH_KEYBOARD, ToggleKeyboardHook, module, thread);
    const HHOOK mouse = SetWindowsHookExW(
        WH_MOUSE, ToggleMouseHook, module, thread);
    if (!keyboard || !mouse) {
        if (keyboard) UnhookWindowsHookEx(keyboard);
        if (mouse) UnhookWindowsHookEx(mouse);
        return;
    }
    g_toggleKeyboardHook = keyboard;
    g_toggleMouseHook = mouse;
    g_toggleWindow = window;
    g_toggleWindowThread = thread;
}
...
{
    EnsureToggleHooks(static_cast<HWND>(gameWindow));
}

void RemoveToggleHotkeyHook() noexcept
{
    RemoveToggleHooks();
}
...
void DrawSettingsPanel(float menuScale)
{
    ...
    if (appearanceOpen)
    {
        ...
        showTooltipOnHover(
            "Font",
            "Choose the typeface used for floating damage numbers.\n\nBundled fonts are embedded in plugin-misc. Defaults to Akaya Telivigala.");
